#!/usr/bin/env bash
#
# Sync your commits from the rdev-erp-sync repo into a personal GitHub commit log.
...
PERSONAL_REPO_DIR="/Users/mac/ITC/COMMIT/rdev-erp-sync-commit-log"
PERSONAL_REPO_REMOTE="https://github.com/imtyaz-17/rdev-erp-sync-commit-log.git"
...
# Branch names may contain '/' (e.g. feature/foo); sanitize for flat filenames.
BRANCH_SLUG="${BRANCH//\//-}"
...
LOG_FILE="$PERSONAL_REPO_DIR/commit_log.$BRANCH_SLUG.txt"
LAST_COMMIT_FILE="$PERSONAL_REPO_DIR/.last_commit.$BRANCH_SLUG"
...
EOF
  ...
  git -C "$PERSONAL_REPO_DIR" add README.md "$(basename "$LOG_FILE")" "$SCRIPT_NAME" ".last_commit.$BRANCH_SLUG"
...
ensure_log_branch() {
  ...
  current_branch="$(git -C "$PERSONAL_REPO_DIR" rev-parse --abbrev-ref HEAD)"

  # Per-branch log files avoid shared-file conflicts; still stash if dirty so checkout never aborts.
  if ! git -C "$PERSONAL_REPO_DIR" diff --quiet \
    || ! git -C "$PERSONAL_REPO_DIR" diff --cached --quiet \
    || [[ -n "$(git -C "$PERSONAL_REPO_DIR" ls-files --others --exclude-standard)" ]]; then
    warn "Stashing uncommitted changes in personal log repo before branch switch"
    git -C "$PERSONAL_REPO_DIR" stash push -u -m "commit-log-sync: auto-stash $(date '+%Y-%m-%d %H:%M:%S')"
  fi

  if [[ "$current_branch" != "$BRANCH" ]]; then
    if git -C "$PERSONAL_REPO_DIR" show-ref --verify --quiet "refs/heads/$BRANCH"; then
      git -C "$PERSONAL_REPO_DIR" checkout "$BRANCH"
    elif git -C "$PERSONAL_REPO_DIR" show-ref --verify --quiet "refs/remotes/origin/$BRANCH"; then
      git -C "$PERSONAL_REPO_DIR" checkout -b "$BRANCH" --track "origin/$BRANCH"
    else
...
main() {
  ...
  git -C "$PERSONAL_REPO_DIR" add "$(basename "$LOG_FILE")" ".last_commit.$BRANCH_SLUG"
